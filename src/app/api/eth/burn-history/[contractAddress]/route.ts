...
// Define proper types for ETHERSCAN API responses
type ETHERSCANApiResponse = {
    status: string;
    message: string;
    result: ETHERSCANTransaction[] | string; // result can be a string when there's an error
};
...
const ETHERSCAN_API_KEY = process.env.ETHERSCAN_API_KEY || "1E2JNG6XDN9SB1HXKHVJS6U1ZVR8GR1GFC";
const ETHERSCAN_API_URL = "https://api.etherscan.io/v2/api?chainid=1";
   ...
   // console.log('Fetching from ETHERSCAN URL:', url);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: "ETHERSCAN API request failed", message: `HTTP ${response.status}` },
          { status: 500 }
        );
      }
    ...
    //  console.log('ETHERSCAN API message:', data.message);
      ...
      return NextResponse.json(
        { error: "Failed to fetch from ETHERSCAN", message: (fetchError as Error).message },
        { status: 500 }
      );
    }
    ...
    if (!Array.isArray(data.result)) {
      console.error("Unexpected ETHERSCAN response format:", data.result);
      return NextResponse.json(
